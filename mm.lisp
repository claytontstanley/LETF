...
(defmethod% validate-full-combinatorial ((obj session-class))
  ...
  (with-pandoric (configFileWdLST) #'args
	...
	(multiple-value-bind (qFloor rFloor) (ffloor (- (third nums) (first nums)) (second nums))
	  (declare (ignore qFloor))
	  (multiple-value-bind (qCeil rCeil) (fceiling (- (third nums) (first nums)) (second nums))
	    (declare (ignore qCeil))
	    (assert (< (min (abs rFloor) (abs rCeil)) .000001) 
		    nil "(~a-~a)/~a not a whole number in line IV=~a" (third nums) (first nums) (second nums) line)))))
