(defun run-model (&key (declarative-finst-span))
  (sgp-fct (list :declarative-finst-span declarative-finst-span))


  (format t "~%~%~%~a~%~%~%" (coerce (car (sgp :bold-inc)) 'double-float))

  (let ((data (mapcar #'cdr (run-block))))

  		 
    (format t "condition-0.00=~a~%" (nth 0 data))
    (format t "condition-0.15=~a~%" (nth 1 data))
    (format t "condition-0.30=~a~%" (nth 2 data))
    (format t "condition-1.00=~a~%" (nth 3 data))))
