...
;;;             : [X] What's the right attended status for something that's no
...
(defclass vision-module (attn-module)
  ((visicon :accessor visicon :initarg :visicon :initform (make-hash-table :test #'eq))
...
(extend-chunks visual-hash-key :copy-function copy-tree)
...
(defmethod process-display ((devin device-interface) 
  ...
  (let ((tempicon nil)
        (temp-table (when (test-feats vis-mod) (make-hash-table :test 'equal)))
    ...
    (setf tempicon (flatten (build-vis-locs-for (device devin) vis-mod)))
    ...
    (dolist (x tempicon)
      (if (and (chunk-p-fct x)
          ...
          (progn
            ...
            (when (test-feats vis-mod)
              (let ((key (hash-chunk-contents x)))
                (setf (chunk-visual-hash-key x) key)
                (push x (gethash key temp-table)))
              ))
        ...
        (progn 
          ...
          (setf tempicon (remove x tempicon)))))
    ...
    (unless clear
      ...
      (maphash (lambda (key val)
                 (unless (or (find key tempicon)   ;; User provided the same chunkname
                             (and (test-feats vis-mod) ;; feature match desired
                                  ;; don't care if the key is valid, do I? (chunk-p-fct key)    
                                  (let ((potential (gethash (chunk-visual-hash-key val) temp-table)))
                                    (some (lambda (x) (chunk-slot-equal (chunk-real-visual-value val) (chunk-real-visual-value x))) potential))))
                   ...
                   ;; Take it out of the feature table if used
                   (when (test-feats vis-mod)
                     (setf (gethash (chunk-visual-hash-key val) (feat-table vis-mod))
                       (remove val (gethash (chunk-visual-hash-key val) (feat-table vis-mod)))))
    ...
    (mapcar #'(lambda (vl) (enter-into-visicon vl vis-mod)) tempicon)
...
(defmethod update-cursor-feat ((devin device-interface) (vis-mod vision-module))
  (when (and (with-cursor-p devin) (device devin))
    (let ((new-pos (get-mouse-coordinates (device devin)))
          (cur-crsr (first (find-matching-chunks (define-chunk-spec isa visual-location kind cursor)
                                                 :chunks (visicon-chunks vis-mod)))))
      (if (null cur-crsr)
        (awhen (cursor-to-vis-loc (device devin))
               (enter-into-visicon it vis-mod))
        (when (not (vpt= new-pos (xy-loc cur-crsr)))
          (fast-set-chunk-slot-value-fct cur-crsr 'screen-x (px new-pos))
          (fast-set-chunk-slot-value-fct cur-crsr 'screen-y (py new-pos))
          
          ;; since the feature is being updated directly
          ;; always invalidate any finst it may have
...
(defmethod update-attended-loc ((vis-mod vision-module))
  ...
  (when (or (and (currently-attended vis-mod)
                 (or (not (chunk-p-fct (currently-attended vis-mod)))
                     (not (object-present-p vis-mod (currently-attended vis-mod)))))
...
(defun test-attended (attended-spec chunk)
  "Assume it's a visual-location chunk, but could be either
   the key or value from the visicon"
  (let* ((vm (get-module :vision))
         (visicon-key (chunk-visicon-entry chunk))
         (visicon-value (gethash visicon-key (visicon vm)))
         (value (third attended-spec))
         (result nil)
         (marker (when visicon-value (feat-attended visicon-value vm))))
...
(defun enter-into-visicon (vis-loc vis-mod)
  ...
  (let* ((existing (or (gethash vis-loc (visicon vis-mod))  ;same name
                       (and (test-feats vis-mod)
                            (let* ((key (aif (chunk-visual-hash-key vis-loc)
                                             it
                                             (setf (chunk-visual-hash-key vis-loc) (hash-chunk-contents vis-loc))))
                                   (potential (gethash key (feat-table vis-mod))))
                              (some (lambda (x) (when (chunk-slot-equal (chunk-real-visual-value vis-loc) (chunk-real-visual-value x)) x)) potential)))))
    ...
    (when existing
      ...
      (when (test-feats vis-mod)
        (setf (gethash (chunk-visual-hash-key existing) (feat-table vis-mod))
          (if (purge-visicon vis-mod)
              (remove existing (gethash (chunk-visual-hash-key existing) (feat-table vis-mod)))
            (remove existing (gethash (chunk-visual-hash-key existing) (feat-table vis-mod)) :test 'chunk-slot-equal :key 'chunk-real-visual-value))))
        
      ;; transfer any finst from the old to the new
      
      (let ((finst (find (chunk-visicon-entry existing) (finst-lst vis-mod) :key 'id)))
        (when finst
          (setf (id finst) vis-loc)))
      ...
      ;; Update the currently-attended marker with the new feature item
    ...
    (setf (gethash vis-loc (visicon vis-mod)) entry)
    
    (when (test-feats vis-mod)
      (push entry (gethash (chunk-visual-hash-key vis-loc) (feat-table vis-mod))))
    
    (setf (chunk-visual-tstamp entry) tstamp)
    (setf (chunk-visicon-entry entry) vis-loc)
    ...
    (setf (chunk-visicon-entry vis-loc) vis-loc)
...
(defclass finst ()
  ((id :accessor id :initarg :id :initform nil) ;; the visicon key chunk-name...
...
(defmethod check-finsts ((vis-mod vision-module))
  (setf (finst-lst vis-mod)
    (delete-if #'(lambda (f) 
                   (let ((c (id f)))
                       (or (not (chunk-p-fct c))
                           (and (not (synthed-from f))
                                (not (object-present-p vis-mod c)))
                           (and (synthed-from f)
                                (not (some (lambda (x) (and (chunk-p-fct x) (object-present-p vis-mod x))) (synthed-from f))))
                           (> (- (mp-time) (tstamp f))
                              (finst-span vis-mod)))))
                   (finst-lst vis-mod))))
...
(defmethod feat-attended (loc vis-mod)
  (if (eq (chunk-visual-new-p loc) 'NEW)
    'NEW
    (if (find (chunk-visicon-entry loc) (finst-lst vis-mod) :key (lambda (x) (cons (id x) (synthed-from x))) :test 'member)
        t
      nil)))

(defun add-finst (vis-mod loc-or-obj) 
  (let ((name (if (gethash loc-or-obj (visicon vis-mod)) loc-or-obj (chunk-visicon-entry loc-or-obj))))
    (if name
        (progn
          (aif (find name (finst-lst vis-mod) :key 'id)
               (setf (tstamp it) (mp-time))
               
               (push
                (make-instance 'finst :id name :tstamp (mp-time)
          ...
          (if (chunk-synth-feat loc-or-obj)
            ...
            (setf (chunk-visual-new-p (gethash name (visicon vis-mod))) nil))
...
(defun remove-finst (vis-mod loc-or-obj) 
  (let ((name (if (gethash loc-or-obj (visicon vis-mod)) loc-or-obj (chunk-visicon-entry loc-or-obj))))
    ...
    (setf (finst-lst vis-mod) (remove name (finst-lst vis-mod) :key 'id))
    (setf (finst-lst vis-mod) (remove name (finst-lst vis-mod) :key 'synthed-from))))
...
(defmethod object-present-p ((vis-mod vision-module) obj-id)
  (if (chunk-synth-feat obj-id)
      ...
      (every (lambda (x) (and (chunk-p-fct x) (object-present-p vis-mod x))) (chunk-synth-feat obj-id))
    
    (if (gethash (chunk-visicon-entry obj-id) (visicon vis-mod))
        t
      nil ;; taking this out for now... (find-matching-chunks (chunk-name-to-chunk-spec (chunk-visicon-entry obj-id)) :chunks (visicon-chunks vis-mod))
      )))
...
(defmethod attend-to-object ((vis-mod vision-module) obj &key (requested t))
  ...
  (aif (member (icon-entry vis-mod obj) (finst-lst vis-mod) :key #'id)
...
(defmethod determine-focus-dmo ((vis-mod vision-module) (dmo-lis list) feat )
  ...
  (aif (member (chunk-visicon-entry feat) dmo-lis :key #'chunk-visicon-entry)
       (first it)
  ...
  (let (accum)
    (dolist (x x-ls (nreverse accum))
      (dolist (feat (text-feats (feat-match-xy (visicon-chunks vis-mod) (vector x y))))
        (setf accum (append (featlis-to-chunks vis-mod (vector x y) (list (chunk-visicon-entry feat))) accum))))))
...
(defmethod synthesize-letter ((vis-mod vision-module) (feats list))
  (let ((base-feat (first feats))
    ...
    (setf (chunk-visicon-entry return-chunk) return-chunk)
    (setf (chunk-synth-feat return-chunk) feats)
    return-chunk))
...
(defmethod synthesize-word ((vis-mod vision-module) (loc-lis list) 
  ...
  (let ((return-chunk nil)
    ...
    (when letter-chunks
      ...
      (setf return-chunk
        (car (define-chunks-fct `((isa text value ,(word-accum 
...
(defmethod synthesize-phrase ((vis-mod vision-module) (feature-locs list) (loc symbol))
  (let ((xy-loc (xy-loc loc))
    ...
    (when word-chunks
      ...
      (setf (chunk-synth-feat return-chunk) (apply #'append (mapcar (lambda (x)
                                                                      (if (eq x (chunk-visicon-entry x))
                                                                          (chunk-synth-feat x)
                                                                        (list (chunk-visicon-entry x))))
                                                              word-chunks)))
...
(defun nearest-feat (vis-mod feat-list xy-loc)
  ...
  (when feat-list
    (let* ((feat-chunks (mapcan (lambda (x) (aif (gethash (chunk-visicon-entry x) (visicon vis-mod)) (list it) nil)) feat-list))
           ...
           (the-dist nil))
      (dolist (feat (rest feat-chunks) (mapcar 'chunk-visicon-entry outlis))
...
(defmethod start-tracking ((vis-mod vision-module))
   (if (null (currently-attended vis-mod))
    ...
    (progn
      (change-state vis-mod :exec 'BUSY)
      (setf (tracked-obj vis-mod) (chunk-visual-object (currently-attended vis-mod)))
...
(defmethod update-tracking-mth ((vis-mod vision-module) &optional from-proc-display)
  ...
  (let ((devin (current-device-interface)))
    ...
    (let* ((new-loc ;; the feature's chunk name (not the copy)
            ...
            (if from-proc-display
              ...
              ;; proc-display wasn't called
              ;; so need to get the new feature info directly.
              ;; The visicon is not updated by the tracking - so
              ;; some things may be out of sync until the next
              ;; proc-display.
              
              (if (tracked-obj vis-mod)
                  
                  ;; It's the old style tracking of a screen object
                  ;; instead of a feature chunk.
                  
                  ;; Not the recommended way to do things now, but
                  ;; since the legacy devices still work that
                  ;; way it needs to be suppoted.
                  
                  ;; explicitly call build-vis-locs-for on
                  ;; the object.
                  
                  (let ((new-feature (build-vis-locs-for (tracked-obj vis-mod) vis-mod)))
                    (if (listp new-feature)
                        (car new-feature)
                      new-feature))
                
                ;; just use the same visual-location chunk from the user
      ...
      (setf (tracked-obj-last-feat vis-mod) new-loc)
      ...
      (let ((new-obj (vis-loc-to-obj (aif (chunk-visual-object new-loc)
        ...
        (setf (currently-attended vis-mod) new-loc)
        ...
        (let ((new-buffer-loc (if from-proc-display 
                                  (gethash new-loc (visicon vis-mod))
                                (copy-chunk-fct new-loc))))
          
          
          ;; when there isn't a screen-pos value in the new object set it to the new buffer chunk 
          ;; which will probably be overwritten anyway...
          ...
          (aif (member (chunk-visicon-entry new-loc) (finst-lst vis-mod) :key #'id)
               (setf (tstamp (first it)) (mp-time))
     ...
     (case slot
       ...
       (attended
        (let* ((vis-loc-chunk (buffer-read 'visual-location))
               (found (and vis-loc-chunk (chunk-visicon-entry vis-loc-chunk))))
          ...
          (when (and vis-loc-chunk found)
            ...
            (let ((test (test-attended (list '= :attended value) found)))
                 (and test (object-present-p vis-mod found))))))))))
  ...
  (if (consp param)
    (case (car param)
      ...
      (:optimize-visual
       ...
       ;(when (null (cdr param))
       ;  (print-warning "Optimize visual cannot currently be set to nil with the new vision module."))
       ;t
