...
proc select_vert_graphic_trace {} {
    ...
    wm withdraw $win
    wm title $win "Vertical Graphic Trace"
    ...
    button $win.stop_gt \
      -command "draw_vert_items $win" -text "Get Trace" -font button_font
    ...
    button $win.redisplay -command "display_vert_data $win 1" -text "Redisplay" -font button_font
...
proc display_vert_data {win level} {
    ...
    foreach x $result {
      ...
      switch [lindex $x 0] {
        ...
        rectangle {
          ...
          if {[lindex $x 8] != "nil"} {
            $win.frame.canvas bind $box_name <Enter> "set $win.textvar \"[expr 0.001 * ([lindex $x 4] - [lindex $x 2])]: [expr 0.001 * [lindex $x 2]] - [expr 0.001 * [lindex $x 4]]\" 
        ...
        if {$pop_up == 1} {
          $win.frame.canvas bind $box_name <ButtonPress> "v_trace_p_view $win [lindex $x 6]"
        } elseif {$pop_up == 2 && [lindex $x 7] != "nil"} {
          $win.frame.canvas bind $box_name <ButtonPress> "v_trace_dm_view $win [lindex $x 7]"
        }
...
proc v_trace_dm_view {win chunk} {
