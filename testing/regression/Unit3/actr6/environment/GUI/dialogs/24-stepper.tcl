  ...
  if {[winfo exists .stepper] == 1} {
    ...
    send_environment_cmd \
      "create simple-handler .stepper stepper_temp \
         (lambda (x) (declare (ignore x)) (if *running-actr* 1 (if (get-module :environment) 0 2))) ()"
    ...
    tk_messageBox -icon info -type ok -title "Stepper" \
                  -message "You cannot open the stepper while the model is running."

      remove_handler .stepper
    } elseif {$stepper_temp == 2} {

    tk_messageBox -icon info -type ok -title "Stepper" \
                  -message "The model was loaded before the environment was connected.  You must reload the model to synchronize them."
   ...
   checkbutton .stepper.stepper_tutor -text "Tutor Mode" -font checkbox_font \
    ...
    send_environment_cmd \
      "create simple-handler .stepper.stepper_tutor stepper_waiter \
         (lambda (x) (declare (ignore x)) (setf *stepper-viewer-mode* :production)) nil"
  ...
  send_environment_cmd \
    "update [get_handler_name .stepper.stepper_tutor] \
            (lambda (x) (declare (ignore x)) \
                    (if (= $stepper_tutor 1) \
                       (progn \
                          (setf *stepper-viewer-mode* :tutor) \
            ...
            while {[set indx [.stepper.prod_frame.f3.f.text search "=" $strt end]] != ""} { 
              ...
              if {$var == "=="} {
                ...
                .stepper.prod_frame.f3.f.text tag bind $t_name <1> {
                  ...
                  if {[get_tutor_response $var $side $buf]} {
               
                    wait_for_non_null tutor_ans
                
                    if {$tutor_ans != "NoAnswer"} {

                      array unset tutor_bindings $t_name

                      .stepper.prod_frame.f3.f.text configure -state normal
                      .stepper.prod_frame.f3.f.text delete $strt $w_end
                      .stepper.prod_frame.f3.f.text tag delete $t_name
                      .stepper.prod_frame.f3.f.text insert $strt $tutor_ans
...
proc get_tutor_response {word side buf} {
  if {[winfo exists .tutor_response] == 1} {
    ...
    bind .tutor_response.entry <Key-Return> "accept_tutor_response $word"
...
proc accept_tutor_response {word} {
  global tutor_entry
  global stepper_temp    
  global tutor_help
  global tutor_ans
