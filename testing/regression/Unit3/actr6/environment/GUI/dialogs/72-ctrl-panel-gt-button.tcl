...
proc select_graphic_trace {} {
    ...
    wm withdraw $win
    wm title $win "Horizontal Graphic Trace"
    ...
    send_environment_cmd \
      "create simple-handler $win.note $win.dummy \
         (lambda (x) (declare (ignore x)) (no-output (sgp :save-buffer-trace t)) nil) (reset)"


    bind $win.note <Destroy> "remove_handler $win.note"

    button $win.stop_gt -command "draw_horiz_items $win" -text "Get trace" -font button_font

    send_environment_cmd \
      "create list-handler $win.stop_gt $win.return \
         (lambda (x) (declare (ignore x))) ()"

    bind $win.stop_gt <Destroy> "remove_handler $win.stop_gt"

    button $win.redisplay -command "display_horiz_data $win 1" -text "Redisplay" -font button_font
    ...
    entry $win.min -textvariable $win.min_extent -text "" 
    entry $win.max -textvariable $win.max_extent -text "" 
...
proc display_horiz_data {win level} {
    ...
    switch [lindex $x 0] {
      ...
      rectangle {
        ...
        $win.frame.canvas create rectangle [lindex $x 1] [lindex $x 2] [lindex $x 3] [lindex $x 4] -width 1 -fill [lindex $x 5] -outline black -tag [list trace_items $box_name]
          ...
          if {[lindex $x 8] != "nil"} {
            $win.frame.canvas bind $box_name <Enter> "set $win.textvar \"[expr 0.001 * ([lindex $x 3] - [lindex $x 1])]: [expr 0.001 * [lindex $x 1]] - [expr 0.001 * [lindex $x 3]]\" 
        ...
        if {$pop_up == 1} {
          $win.frame.canvas bind $box_name <ButtonPress> "h_trace_p_view $win [lindex $x 6]"
        } elseif {$pop_up == 2 && [lindex $x 7] != "nil"} {
          $win.frame.canvas bind $box_name <ButtonPress> "h_trace_dm_view $win [lindex $x 7]"
        }
...
proc h_trace_dm_view {win chunk} {
