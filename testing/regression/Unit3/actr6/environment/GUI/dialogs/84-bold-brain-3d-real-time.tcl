...
global gvar
 ...
 proc bb_Init {name title init} {
   ...
   if {[winfo exists $name] == 1} {
      ...
      wm geometry $win [get_configuration $name $win]
...
proc create_brain_real_time_view {win} {
   ...
   send_environment_cmd \
     "create simple-funcall-handler $win.c \"update_brain_view $win\" \
        start-brain-scan (reset)"
...
set voxel_colors [list "#f00" "#090" "#00f" "#aa0" "#0aa" "#a0a" "#0e0" "#36a"]
 ...
 proc main_bold_brains {win title init} {
...
proc start_realtime_brain_view {} {
  ...
  main_bold_brains .bold_brain_3d_real "3d BOLD run-time viewer" create_brain_real_time_view
}
