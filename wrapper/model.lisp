...
(defparameter *inputLines* (get-lines (get-arg 1) #\;))
(defparameter *platform* (get-arg 0))
(assert *inputLines*)
(assert *platform*)
(run-the-loads *platform*)
(dolist (inputLine *inputLines*)
  (let ((tbl (make-hash-table :test 'equalp)))
    (mapc 
     #'(lambda (str)
	 (let ((index (find-in-string str #\=)))
	   (assert (equal (length index) 1))
	   (setf (gethash (get-word (subseq str 0 (first index))) tbl) 
		 (eval (read-from-string (get-word (subseq str (+ 1 (first index)) (length str))))))))
     (get-words inputLine #\,)) 
    (run-model tbl)))
(quit)